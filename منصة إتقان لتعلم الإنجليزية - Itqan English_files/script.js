  ...
  try {
    ...
    if (data.questions && Array.isArray(data.questions) && data.questions.length > 0) {
      ...
      q.options.forEach((opt, idx) => {
        ...
        optionBtn.className = "w-full p-4 border border-slate-100 hover:border-brand-blue bg-white rounded-xl text-right text-xs font-bold font-sans cursor-pointer transition-all hover:bg-slate-50 flex items-center justify-between";
        optionBtn.onclick = () => submitStoryComprehensionChoice(idx, q.answerIndex, q.explanation);
        optionBtn.id = `story-option-${idx}`;
        
        optionBtn.innerHTML = `
          <span>${opt}</span>
          <span class="w-4 h-4 rounded-full border border-slate-200"></span>
        `;
